...
This document describes version 0.40 of YAML::Syck, released
March 31, 2006.
...
This module is not currently pluggable to the new L<YAML> (0.50+) framework,
but that's expected to change in the future.

Tag names such as C<!perl/Foo> is blessed into the package C<Foo>, but the
C<!hs/Foo> tag is blessed into C<hs::Foo>.  Note that this holds true even if
the tag contains non-word characters; for example, C<!haskell.org/Foo> is
blessed into C<haskell.org::Foo>.  Please use L<Class::Rebless> to cast it into
other user-defined packages.
...
Copyright 2005, 2006 by Audrey Tang E<lt>autrijus@autrijus.orgE<gt>.

The F<libsyck> code bundled with this library by I<why the lucky stiff>,
under a BSD-style license.  See the F<COPYING> file for details.
